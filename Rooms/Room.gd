...
const ENEMY_SCENES: Dictionary = {
	...
	"RoyalKnight": preload("res://Characters/Enemies/royalKnight/royalKnight.tscn")
}
...
func _spawn_enemies() -> void:
	for enemy_position in enemy_positions_container.get_children():
		...
		if boss_room:
			...
			var random_value = randi() % 3  # Generate a random number: 0, 1, or 2
			if random_value == 0:
				enemy = ENEMY_SCENES.RoyalKnight.instantiate()
			elif random_value == 1:
				enemy = ENEMY_SCENES.GOBLIN.instantiate()
			else:
				enemy = ENEMY_SCENES.FLYING_CREATURE.instantiate()  # Third enemy type
